class Message::Createchat2Controller < ApplicationController
    def index
        ...
        @message.save
        @chat = Personalchat.joins(:user).select('content, send_id, user_id, name, image_name').where('send_id = ? and user_id = ? or send_id = ? and user_id = ?' , "#{params[:user]}", "#{params[:room]}", "#{params[:room]}", "#{params[:user]}").order(id: "ASC")
